    ...
    if (isNotEmptyString(sign)) {
        getFPByfingerprint(sign).then(fp => {
            ...
            if (fp == null) {
                countDocuments().then(num => {
                    if (num > USER_MAX_NUMBER) {
                        let msg = 'Error: 免费用户已达上限，定制化域名联系客服QQ2460866284 | Free users have reached the limit'
                        res.status(200).send({ status: 'Error', message: msg ?? 'Please authenticate.', data: null })
                        res.end()
                    } else {
            ...
            // sign 找到了但免费次数已经没有了
                ...
                let msg = 'Error: 已超出每日' + PER_DAY_COUNT + '次免费次数，定制化域名联系客服QQ2460866284 | ' + PER_DAY_COUNT + ' free times per day'
                res.status(200).send({ status: 'Error', message: msg ?? 'Please authenticate.', data: null })
                res.end()
            // sign 找到了而且还有免费次数
            } else {
